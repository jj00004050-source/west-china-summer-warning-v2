...
export default function AreaRevenueZoneMatrix({ rows, comparisonRows, sameLeadRows, channelRows, onRevenueZone }: { rows: MetricRow[]; comparisonRows: ComparisonRow[]; sameLeadRows: SameLeadComparisonRow[]; channelRows: SnapshotRecord[]; onRevenueZone: (name: string) => void }) {
  ...
  const exportRows = async () => {
    ...
    const data = areas.flatMap(area => {
      const parent = { 层级: '片区', 片区: area.name, 收益管理商圈: '', 门店数: area.rows.length, 可售房: area.availableRooms, 预订房间数: area.bookedRooms, 预订率: area.bookingRate, 预订率环比: area.bookingRateChange, 同期开盘预订率: area.sameLeadBookingRate, 同期开盘预订率差异: area.sameLeadBookingRateGap, 同期OCC: area.lastOcc, 在手ADR: area.adr, 在手ADR环比: area.adrChange, 同期开盘在手ADR: area.sameLeadAdr, 同期开盘ADR差异: area.sameLeadAdrGap, 理论RP: area.rp, 理论RP环比: area.rpChange, 同期开盘理论RP: area.sameLeadRp, 同期开盘理论RP差异: area.sameLeadRpGap, 同期RP: area.lastRp, 高风险: area.highCount }
      const zones = aggregateBy(area.rows, 'revenueZone', comparisonRows.filter(r => r.area === area.name), sameLeadRows.filter(r => r.area === area.name)).map(zone => ({ 层级: '收益管理商圈', 片区: area.name, 收益管理商圈: zone.name, 门店数: zone.rows.length, 可售房: zone.availableRooms, 预订房间数: zone.bookedRooms, 预订率: zone.bookingRate, 预订率环比: zone.bookingRateChange, 同期开盘预订率: zone.sameLeadBookingRate, 同期开盘预订率差异: zone.sameLeadBookingRateGap, 同期OCC: zone.lastOcc, 在手ADR: zone.adr, 在手ADR环比: zone.adrChange, 同期开盘在手ADR: zone.sameLeadAdr, 同期开盘ADR差异: zone.sameLeadAdrGap, 理论RP: zone.rp, 理论RP环比: zone.rpChange, 同期开盘理论RP: zone.sameLeadRp, 同期开盘理论RP差异: zone.sameLeadRpGap, 同期RP: zone.lastRp, 高风险: zone.highCount }))
      return [parent, ...zones]
    })
  ...
  return <section className="light-card province-city-matrix">
    <div className="light-card-head"><div><h2>片区 / 收益管理商圈经营矩阵</h2><p>点击片区展开收益管理商圈 · 点击商圈联动门店 · 点击表头排序</p></div><div className="matrix-head-actions"><ChannelColorLegend/><button className="table-export" onClick={exportRows}><Download/>导出矩阵</button></div></div>
    <div className="province-table matrix-scroll"><table><thead><tr>{th('name','片区 / 收益管理商圈')}{th('rows','门店数')}{th('availableRooms','可售房')}{th('bookingRate','预订率')}{th('lastOcc','同期OCC')}<th>OCC缺口</th>{th('adr','在手ADR')}{th('rp','理论RP')}{th('lastRp','同期RP')}<th>RP缺口</th>{th('highCount','高风险')}<th>渠道占比</th></tr></thead>
      ...
      })}<tr className="total"><td><b>当前筛选合计</b></td>{cells(total, rows.length, rows)}</tr></tbody></table></div>
  </section>
}
