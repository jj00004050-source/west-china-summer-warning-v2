...
export default function ProvinceCityMatrix({ rows, comparisonRows, sameLeadRows, channelRows, onCity }: { rows: MetricRow[]; comparisonRows: ComparisonRow[]; sameLeadRows: SameLeadComparisonRow[]; channelRows: SnapshotRecord[]; onCity: (city: string) => void }) {
  ...
  const exportRows = async () => {
    ...
    const data = provinces.flatMap(p => {
      const province = { 层级: '省区', 省区: p.name, 城市: '', 门店数: p.rows.length, 可售房: p.availableRooms, 预订房间数: p.bookedRooms, 预订率: p.bookingRate, 预订率环比: p.bookingRateChange, 同期开盘预订率: p.sameLeadBookingRate, 同期开盘预订率差异: p.sameLeadBookingRateGap, 同期OCC: p.lastOcc, 在手ADR: p.adr, 在手ADR环比: p.adrChange, 同期开盘在手ADR: p.sameLeadAdr, 同期开盘ADR差异: p.sameLeadAdrGap, 理论RP: p.rp, 理论RP环比: p.rpChange, 同期开盘理论RP: p.sameLeadRp, 同期开盘理论RP差异: p.sameLeadRpGap, 同期RP: p.lastRp, 高风险: p.highCount }
      const cities = aggregateBy(p.rows, 'city', comparisonRows.filter(r => r.province === p.name), sameLeadRows.filter(r => r.province === p.name)).map(c => ({ 层级: '城市', 省区: p.name, 城市: c.name, 门店数: c.rows.length, 可售房: c.availableRooms, 预订房间数: c.bookedRooms, 预订率: c.bookingRate, 预订率环比: c.bookingRateChange, 同期开盘预订率: c.sameLeadBookingRate, 同期开盘预订率差异: c.sameLeadBookingRateGap, 同期OCC: c.lastOcc, 在手ADR: c.adr, 在手ADR环比: c.adrChange, 同期开盘在手ADR: c.sameLeadAdr, 同期开盘ADR差异: c.sameLeadAdrGap, 理论RP: c.rp, 理论RP环比: c.rpChange, 同期开盘理论RP: c.sameLeadRp, 同期开盘理论RP差异: c.sameLeadRpGap, 同期RP: c.lastRp, 高风险: c.highCount }))
